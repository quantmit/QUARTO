...
params:
  season_params:
    ...
    value: ["Data/season_1.RData"]  # Default value(s), change for different seasons
    input: select
    choices:
      - ../Data/season_1.RData
      - ../Data/season_2.RData
...
plot(season_data$no_season, season_data$viewers, type="l", col="blue", lwd=2, xlab = "Episode number", ylab = "1st day TV viewers in the US (millions)")


```

```{r}
above_average_episodes <- season_data %>% filter(viewers > mean(viewers))
knitr::kable(above_average_episodes, format = "html", caption = "Episodes with Above-Average Viewership")
```
